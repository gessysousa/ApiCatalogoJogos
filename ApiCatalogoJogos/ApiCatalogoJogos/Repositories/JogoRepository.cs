...
namespace ApiCatalogoJogos.Repositories
{
    ...
    {
        private static Dictionary<Guid, Jogo> jogos = new Dictionary<Guid, Jogo>()//fica na memória e não é destruído para fazer de conta que é um banco de dados
        {
        ...
        {
            return Task.FromResult(jogos.Values.Skip((pagina - 1) * quantidade).Take(quantidade).ToList());//skip pula a quantidade de registros que você quer
        }

        public Task<Jogo> Obter(Guid id) //obter por id
        {
            if (!jogos.ContainsKey(id))
                return Task.FromResult<Jogo>(null);

            return Task.FromResult(jogos[id]);//como não se tem uma conexão com o banco de dados propriamente dita, taskfromresult cria uma task do tipo que eu to passando e compila esse código  
        }

        public Task<List<Jogo>> Obter(string nome, string produtora)
        {
            return Task.FromResult(jogos.Values.Where(jogo => jogo.Nome.Equals(nome) && jogo.Produtora.Equals(produtora)).ToList());//estudar link, where filtra a lista e o tolist lista os itens que foram filtrados no where
        }
